...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/businesses/tag/<tag>` | Filter by tag (uses NoSQL array query) |
| GET | `/businesses/price?min=&max=` | Filter by product price range (aggregation) |
...
| POST | `/businesses` | Insert a new business (flexible-schema write) |
    ...
    - - `GET /graph/related/<biz_id>` — businesses sharing tags / neighborhood / category
      - - `GET /graph/path?source=&target=` — shortest path between any two nodes
        ...
        - 1. Show `seed_db.py` output: "Seeded N businesses into NoSQL store."
          2. 2. Start Flask: `flask --app app run -p 5002`
             3. 3. Start ngrok: `ngrok http 5002`
                4. 4. Run `python access_api.py` — narrate each endpoint response.
                   5. 5. Call out the new endpoints (`/top-rated`, `/stats`, `/open-now`, `/businesses/tag/...`) and explain they are powered by MongoDB queries / aggregation, not in-memory Python filtering.
                      6. 6. Finish with a `POST /businesses` to show NoSQL writes — the document count goes up and the new record appears in `/businesses`.
                         7. 
